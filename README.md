...
| `DISCORD_APPLICATION_ID` | Command registration only | Application ID used by `npm run register` or `POST /api/register-commands`. |
...
Open `https://your-bot.vercel.app/api/interactions` in a browser after deploying. It should return a small JSON health response. That confirms the Vercel function exists, but it does **not** mean slash commands have been registered. Use `POST /api/register-commands` or `npm run register` to create the commands in Discord. Discord POST signature verification is still required for real interactions.
