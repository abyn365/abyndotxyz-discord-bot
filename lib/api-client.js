...
export async function updateLocation(city, country) {
  ...
  try {
    return await requestJson('/api/location', locationUpdateRequest(body, `Bearer ${LOCATION_SECRET}`));
  } catch (error) {
    if (error.status !== 401) {
      throw error;
    }

    return requestJson('/api/location', locationUpdateRequest(body, LOCATION_SECRET));
  }
}
...
function locationUpdateRequest(body, authorization) {
  return {
    ...
    headers: {
      ...
      'X-Location-Secret': LOCATION_SECRET,
